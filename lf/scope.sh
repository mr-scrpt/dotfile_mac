...
case "$(file --dereference --brief --mime-type -- "$1")" in
	...
	# image/*) chafa --fill=block --symbols=block -w 9 -c 256 -s 70x"${HEIGHT}" "$1" "$2" "$3" "$4" "$5" "$1";;
	image/*) chafa --fill=block --symbols=block -w 9 -c full --color-space rgb -s 50x"${HEIGHT}" "$1";;
	# text/html) bat --color always "$1";;
